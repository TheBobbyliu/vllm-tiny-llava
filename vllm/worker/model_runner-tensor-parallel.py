...
class ModelRunner:
    ...
    def _prepare_input_metadata_vision(
        self,
        seq_group_metadata_list: List[SequenceGroupMetadata],
        pixel_values: List[torch.Tensor],
        image_token_len: int
    ) -> InputMetadata:
        valid_num = len([1 for val in pixel_values if val is not None])
        if not valid_num:
            return None
    ...
    def execute_llava_model(
        ...
        # input_metadata_vision = self._prepare_input_metadata_vision(seq_group_metadata_list, extra_kwargs.get("pixel_values", []), 752)
        # extra_kwargs["input_metadata_vision"] = input_metadata_vision
